...
class GetAIOrchestrationStatus200ResponseToolsInner(BaseModel):
    """
    ...
    result: Optional[Dict[str, Any]] = Field(default=None, description="Tool result (if complete)")
    error: Optional[StrictStr] = Field(default=None, description="Error message (if failed)")
    __properties: ClassVar[List[str]] = ["executionId", "toolName", "status", "result", "error"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['pending', 'running', 'complete', 'failed']):
            raise ValueError("must be one of enum values ('pending', 'running', 'complete', 'failed')")
        return value
